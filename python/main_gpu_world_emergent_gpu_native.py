...
Single source of truth for the RTX 5090 pipeline: forces, neighbor lists,
Arrhenius probability, and bond creation all remain on the GPU so we avoid the
CPU bottlenecks that killed the previous attempts.
"""
...
def _install_gpu_native_reactions(lmp: lammps, cfg: EmergentConfig) -> None:
    ...
    lmp.command(
        ...
        "iparam 1 1 jparam 1 1".format(
            maxb=cfg.max_bonds_per_atom,
            cutoff=cfg.reaction_radius_nm / 0.34,
        )
    )
